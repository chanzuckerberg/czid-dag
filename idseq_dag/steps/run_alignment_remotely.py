...
class PipelineStepRunAlignmentRemotely(PipelineStep):
    ...
    def run_chunk(self, part_suffix, remote_home_dir, remote_index_dir, remote_work_dir,
        ...
        if service == "gsnap":
            commands = base_str + "{remote_home_dir}/bin/gsnapl -A m8 --batch=0 --use-shared-memory=0 --gmap-mode=none --npaths=100 --ordered -t 36 --min-coverage=30 --max-mismatches=40 -D {remote_index_dir} -d nt_k16 {remote_input_files} > {multihit_remote_outfile}"
        else:
