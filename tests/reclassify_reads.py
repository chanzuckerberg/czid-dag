...
class ReclassifyReadsTest(unittest.TestCase):
    ...
    def test_step_gsnap(self):
        runstep = IdseqStepSetup.get_step_object(ReclassifyReadsTest, "refined_gsnap_out", paired=True)
        runstep.start()
    ...
    def test_step_rapsearch2(self):
        runstep = IdseqStepSetup.get_step_object(ReclassifyReadsTest, "refined_rapsearch2_out", paired=True)
        runstep.start()
        runstep.wait_until_finished()
        # Check results
        # Clean up the folder
